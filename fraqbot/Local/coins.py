...
class Coins(Lego):
    def __init__(self, baseplate, lock, *args, **kwargs):
        ...
        self.handlers = {t: self._handle_coins for t in self.triggers}
        self.handlers['moin'] = self._handle_moins
        ...
        if isinstance(text, str):
            ...
            if any([text.startswith(t) for t in self.triggers]):
                ...
                if (
                    ...
                    message['metadata']['text'] = f'moin {text}'
                    _handle = True
    ...
    def handle(self, message):
        params = re.split(r'\s+', message['metadata']['text'])
        handler = self.handlers.get(params.pop(0))
        if handler:
            handler(message, params)

    # # Coin Handlers
    def _handle_coins(self, message, params):
        handlers = {
            ...
            'tip': self._handle_pay
        }
        handler = handlers.get(params.pop(0))
        if handler:
    ...
    def _handle_pool(self, message, params):
        return self._format_get_pool()

    # # Moin Handlers
    def _handle_moins(self, message, params):
        while self.moiners:
    ...
    def _update_pool(self):
        if not self.next_pool:
            _now = h.now()
            next_pool = self._get_next_pool()
            if not next_pool or next_pool <= _now:
    ...
    def _mine(self, moiner):
        ...
        if opts and pool_balance and divvy:
            ...
            return amt if self._pay('pool', moiner, amt, 'Happy Moin!') else 0
